...
> **你的套件版本可能和講者不一樣。** `requirements.txt` 是寬鬆的下限，
> 講者台上那台跑的是 pandas 3.0.3 / numpy 2.5.1 / lifelines 0.30.0，
> 你現在裝到的大概是更新的組合。核心數字（0.505 / 706 對 / 0.578）
> 是決定性計算，兩邊會一致。
>
> 唯一看得出差別的地方是 `add_at_risk_counts()`：
> 在講者那台是**壞的**（lifelines 0.30.0 + numpy 2.5.1 的已知相沖），
> 較新的 lifelines 則正常。preflight 會直接告訴你是哪一種——
> 顯示「不可用」時**不要去 debug、不要降版**，Mission 3.2 有寫好的手工替代碼；
> 顯示「可用」就直接用官方函式。兩條路產出的圖是一樣的。
