...
<style>
  ...
  /* Windowed, the generated page caps the player at 1100px, so on a large or
     retina display it sits small in the middle of the screen and the vertical
     space goes unused. Size it the same way fullscreen does — the largest 16:9
     box that fits under the bar — just with a margin so it does not touch the
     edges. */
  /* Fill the window outright rather than fitting a 16:9 box into it. The
     terminal is 105x30, wider than most windows are, so an aspect-ratio box
     always left a band of empty page below it. asciinema-player already
     centres and scales the grid inside whatever box it is given, and the page
     and terminal backgrounds match, so handing it the whole viewport looks
     seamless and picks the same largest glyph size. */
  /* asciinema-player scales the grid to fit its box and then leaves the slack
     at the bottom. Centring it turns "a gap below the recording" into a even
     margin above and below, which is what a letterboxed video looks like. */
  .ap-wrapper { align-items: center !important; justify-content: center !important; }
  ...
  /* The player ships its own fullscreen button, which expands only the
     terminal — hiding this bar and the chapter pane — and resumes playback on
     the way in. There should be one way to go fullscreen, and it is ours, so
     hide theirs. */
  ...
  body.fs #player { width: 100vw !important; height: 100vh !important; }
...
(function () {
  ...
  function onFsChange() {
    ...
    // Entering or leaving fullscreen must not change whether it is playing.
    // The browser can resume on its own — F11, or the OS-level control — so
    // put the state back rather than trusting it to be left alone.
    setTimeout(function () {
      try {
